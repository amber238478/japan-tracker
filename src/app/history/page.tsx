...
export default function HistoryPage() {
  ...
  const totalJPY = tripReceipts.filter(r => r.currency !== 'TWD').reduce((a, r) => a + (r.category === '代買' ? -r.amount : r.amount), 0)
  const totalTWD = tripReceipts.filter(r => r.currency === 'TWD').reduce((a, r) => a + (r.category === '代買' ? -r.amount : r.amount), 0)
    ...
    <main>
      ...
      <div className="page">
        ...
        {Object.entries(grouped).sort(([a], [b]) => b.localeCompare(a)).map(([date, items]) => {
          const dayJPY = items.filter(r => r.currency !== 'TWD').reduce((a, r) => a + (r.category === '代買' ? -r.amount : r.amount), 0)
          const dayTWD = items.filter(r => r.currency === 'TWD').reduce((a, r) => a + (r.category === '代買' ? -r.amount : r.amount), 0)
