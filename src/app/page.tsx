...
export default function Home() {
  ...
  const touchStartX = useRef<number | null>(null)
    ...
    const tripDayJPY = tripDayReceipts.filter(r => r.currency !== 'TWD').reduce((a, r) => a + (r.category === '代買' ? -r.amount : r.amount), 0)
    const tripDayTWD = tripDayReceipts.filter(r => r.currency === 'TWD').reduce((a, r) => a + (r.category === '代買' ? -r.amount : r.amount), 0)
    // 建立旅程日期選單資料
